...
<button onClick={(e) => { e.stopPropagation(); handleEdit(note); }}>Edit</button>
<button onClick={(e) => { e.stopPropagation(); handleDelete(note.id); }}>Delete</button>
</div>
))}
  <Modal isOpen={modal} toggle={() => setModal(!modal)}>
    <ModalHeader toggle={() => setModal(!modal)}>{selectedNote?.title}</ModalHeader>
    <ModalBody>
      {editingNote ? (
